...
<section id="page-classifications" class="page-section space-y-6 hidden">
    ...
    <div class="custom-card p-6 rounded-2xl">
        <div class="overflow-x-auto"><table class="custom-table text-xs" id="tbl-classifications">
            <thead><tr><th class="w-8">ت</th><th>اسم التصنيف</th><th class="text-center w-24">تسلسل العرض</th><th class="text-center w-20">تعديل</th><th class="text-center w-20">حذف</th></tr></thead>
...
async function loadClassifications() {
    ...
    tb.innerHTML = data.map((d,i)=>`<tr class="table-row">
        ...
        <td class="font-bold">
            <input type="text" id="class-name-${d.id}" value="${d.name}" disabled
        ...
        <td class="text-center">
            <input type="number" id="class-order-${d.id}" value="${d.display_order || 0}" disabled
        ...
        <td class="text-center">
            <button onclick="toggleEditClass(${d.id})" id="btn-edit-class-${d.id}" class="w-7 h-7 rounded-lg bg-amber-100 text-amber-600 flex items-center justify-center mx-auto hover-press">
...
function toggleEditClass(id) {
    const inputName = document.getElementById(`class-name-${id}`);
    const inputOrder = document.getElementById(`class-order-${id}`);
    const btn = document.getElementById(`btn-edit-class-${id}`);
    ...
    if (inputName.hasAttribute('disabled')) {
        ...
        if (!name) {
            showToast('الرجاء كتابة اسم التصنيف', 'error');
            return;
        }
        updateClassificationFields(id, name, order);
...
async function updateClassificationFields(id, name, order) {
    try {
