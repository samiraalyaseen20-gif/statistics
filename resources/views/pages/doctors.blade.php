...
<section id="page-doctors" class="page-section space-y-6 hidden">
    ...
    <div class="custom-card p-6 rounded-2xl">
        <div class="overflow-x-auto"><table class="custom-table text-xs" id="tbl-doctors">
            <thead><tr><th class="w-8">ت</th><th>اسم الطبيب</th><th class="text-center w-24">تسلسل العرض</th><th class="text-center w-20">حذف</th></tr></thead>
    ...
    tb.innerHTML = data.map((d,i)=>`<tr class="table-row">
        ...
        <td class="text-center">
            <input type="number" value="${d.display_order || 0}" 
                ...
                onchange="updateDocOrder(${d.id}, '${d.name}', this.value)">
        </td>
        <td class="text-center">
...
async function updateDocOrder(id, name, order) {
    try {
