...
<section id="page-operations" class="page-section space-y-6 hidden">
    <div class="custom-card p-6 rounded-2xl">
        <div class="flex flex-col sm:flex-row gap-3">
            ...
            <select id="inp-op-class" class="w-32 custom-inset border-none rounded-xl py-2.5 px-4 text-xs font-medium focus:outline-none text-text-main">
                <option value="">تصنيف العملية</option>
            </select>
    ...
    <div class="custom-card p-6 rounded-2xl">
        <div class="overflow-x-auto"><table class="custom-table text-xs" id="tbl-operations">
            <thead><tr><th class="w-8">ت</th><th>اسم العملية</th><th class="text-center w-32">التصنيف</th><th class="text-center w-24">تسلسل العرض</th><th class="text-center w-20">حذف</th></tr></thead>
...
async function loadOps() {
    ...
    tb.innerHTML = data.map((d,i)=>{
        const classOpts = globalClassifications.map(c => 
            `<option value="${c.name}" ${c.name === d.classification ? 'selected' : ''}>${c.name}</option>`
        ).join('');
        return `<tr class="table-row">
        <td class="text-center">${i+1}</td>
        <td class="font-bold">${d.name}</td>
        <td class="text-center">
            <select class="w-24 text-center custom-inset border-none focus:outline-none rounded-lg py-1 px-1.5 font-bold text-text-main"
                onchange="updateOp(${d.id}, '${d.name}', this.value, ${d.display_order || 0})">
                <option value="">--</option>
                ${classOpts}
            </select>
        </td>
        <td class="text-center">
            <input type="number" value="${d.display_order || 0}" 
                ...
                onchange="updateOp(${d.id}, '${d.name}', '${d.classification || ''}', this.value)">
        </td>
        <td class="text-center">`
    }).join('');
...
async function addOp() {
    ...
    const classification = document.getElementById('inp-op-class').value;
    const order = document.getElementById('inp-op-order').value || 0;
    if(!name) return showToast('الرجاء كتابة اسم العملية','error');
    if(!classification) return showToast('الرجاء اختيار تصنيف العملية','error');
    try {
        ...
        document.getElementById('inp-op-class').value='';
...
window.initOperationsPage = async function() {
    globalClassifications = await apiFetch('/api/classifications') || [];
    const classSelect = document.getElementById('inp-op-class');
    if (classSelect) {
        classSelect.innerHTML = '<option value="">تصنيف العملية</option>' + 
            globalClassifications.map(c => `<option value="${c.name}">${c.name}</option>`).join('');
    }
    loadOps();
};
</script>
