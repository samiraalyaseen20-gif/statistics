...
<section id="page-countries" class="page-section space-y-6 hidden">
    ...
    <div class="custom-card p-6 rounded-2xl">
        <div class="overflow-x-auto"><table class="custom-table text-xs" id="tbl-countries">
            <thead><tr><th class="w-8">ت</th><th>اسم الدولة</th><th class="text-center w-20">تعديل</th><th class="text-center w-20">حذف</th></tr></thead>
...
async function loadCountries() {
    ...
    tb.innerHTML = data.map((d,i)=>`<tr class="table-row">
        ...
        <td class="font-bold">
            <input type="text" id="country-name-${d.id}" value="${d.name}" disabled
        ...
        <td class="text-center">
            <button onclick="toggleEditCountry(${d.id})" id="btn-edit-country-${d.id}" class="w-7 h-7 rounded-lg bg-amber-100 text-amber-600 flex items-center justify-center mx-auto hover-press">
...
function toggleEditCountry(id) {
    const inputName = document.getElementById(`country-name-${id}`);
    const btn = document.getElementById(`btn-edit-country-${id}`);
    ...
    if (inputName.hasAttribute('disabled')) {
        ...
        if (!name) {
            showToast('الرجاء كتابة اسم الدولة', 'error');
            return;
        }
...
async function updateCountry(id, name) {
    try {
        await apiFetch(`/api/countries/${id}`, 'PUT', { name });
        showToast('تم تحديث اسم الدولة بنجاح');
    } catch (e) {
        showToast('فشل تحديث اسم الدولة', 'error');
    }
}
