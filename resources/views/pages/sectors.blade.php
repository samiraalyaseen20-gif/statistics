...
<section id="page-sectors" class="page-section space-y-6 hidden">
    ...
    <div class="custom-card p-6 rounded-2xl">
        <div class="overflow-x-auto"><table class="custom-table text-xs" id="tbl-sectors">
            <thead><tr><th class="w-8">ت</th><th>اسم القطاع</th><th class="text-center w-20">تعديل</th><th class="text-center w-20">حذف</th></tr></thead>
...
async function loadSectors() {
    ...
    tb.innerHTML = data.map((d,i)=>`<tr class="table-row">
        ...
        <td class="font-bold">
            <input type="text" id="sector-name-${d.id}" value="${d.name}" disabled
        ...
        <td class="text-center">
            <button onclick="toggleEditSector(${d.id})" id="btn-edit-sector-${d.id}" class="w-7 h-7 rounded-lg bg-amber-100 text-amber-600 flex items-center justify-center mx-auto hover-press">
...
function toggleEditSector(id) {
    const inputName = document.getElementById(`sector-name-${id}`);
    const btn = document.getElementById(`btn-edit-sector-${id}`);
    ...
    if (inputName.hasAttribute('disabled')) {
        ...
        if (!name) {
            showToast('الرجاء كتابة اسم القطاع', 'error');
            return;
        }
...
async function updateSector(id, name) {
    try {
        await apiFetch(`/api/sectors/${id}`, 'PUT', { name });
        showToast('تم تحديث اسم القطاع بنجاح');
    } catch (e) {
        showToast('فشل تحديث اسم القطاع', 'error');
    }
}
