...
<section id="page-lab_test_types" class="page-section space-y-6 hidden">
    ...
    <div class="custom-card p-6 rounded-2xl">
        <div class="overflow-x-auto"><table class="custom-table text-xs" id="tbl-lab-test-types">
            <thead><tr><th class="w-8">ت</th><th>نوع التحليل</th><th class="text-center w-20">تعديل</th><th class="text-center w-20">حذف</th></tr></thead>
...
async function loadLabTestTypes() {
    ...
    tb.innerHTML = data.map((d,i)=>`<tr class="table-row">
        ...
        <td class="font-bold">
            <input type="text" id="lab-test-name-${d.id}" value="${d.name}" disabled
        ...
        <td class="text-center">
            <button onclick="toggleEditLabTest(${d.id})" id="btn-edit-lab-test-${d.id}" class="w-7 h-7 rounded-lg bg-amber-100 text-amber-600 flex items-center justify-center mx-auto hover-press">
...
function toggleEditLabTest(id) {
    const inputName = document.getElementById(`lab-test-name-${id}`);
    const btn = document.getElementById(`btn-edit-lab-test-${id}`);
    ...
    if (inputName.hasAttribute('disabled')) {
        ...
        if (!name) {
            showToast('الرجاء كتابة اسم التحليل', 'error');
            return;
        }
        updateLabTestType(id, name);
...
async function updateLabTestType(id, name) {
    try {
        await apiFetch(`/api/lab-test-types/${id}`, 'PUT', { name });
        showToast('تم تحديث اسم التحليل بنجاح');
    } catch (e) {
        showToast('فشل تحديث اسم التحليل', 'error');
    }
}
